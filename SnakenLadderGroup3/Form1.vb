...
Public Class Form1
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles btnRollDice.Click
        'Dice_Show1()
        DicePic(picDie1, ClassDice.getDice)
        setPlayer1(ClassDice.getDice())

        If startCondition = True Then
            player1_Score = setPlayerScore(player1_Score, ClassDice.getDice)
            If player1_Score <= 29 Then
                PlayerPos(picMario, p1_b(player1_Score), player1_Score)
            Else
    ...
    Private Sub btnRollDice2_Click(sender As Object, e As EventArgs) Handles btnRollDice2.Click
        'Dice_Show2()
        DicePic(picDie2, ClassDice.getDice)
    ...
    Public Sub DicePic(img As PictureBox, diceNum As Integer)
        Select Case diceNum
            Case 1
                img.Image = ImageList1.Images("dice1.JPG")
            Case 2
                img.Image = ImageList1.Images("dice2.JPG")
            Case 3
                img.Image = ImageList1.Images("dice3.JPG")
            Case 4
                img.Image = ImageList1.Images("dice4.JPG")
            Case 5
                img.Image = ImageList1.Images("dice5.JPG")
            Case 6
                img.Image = ImageList1.Images("dice6.JPG")
        End Select

    End Sub

    Public Sub PlayerPos(player As PictureBox, place As Point, num As Integer)

        If (num < 30) Then
            player.Location = place
        Else
            If (player1_Score > 30) Then
    ...
    Public Sub setPlayer1(Dice As Integer)
        ...
        'If Game_is_valid = True Then
        '    player1_Score += ClassDice.getDice
        '    Console.WriteLine(player1_Score)
